...
// rotate the values in an array ([0 1 2 3] becomes [1 2 3 0] if we rotate by 1)
                ...
                typename std::iterator_traits<Iterator>::value_type *cache, const size_t cache_size)
    {
        ...
        if (0)
        { // tb: disabled for more display output
...
// standard merge operation using an internal or external buffer
    ...
    void Merge(const RangeI<Iterator> &buffer, const RangeI<Iterator> &A, const RangeI<Iterator> &B,
               const Comparison compare, typename std::iterator_traits<Iterator>::value_type *cache,
               const ssize_t cache_size)
    {
        typedef typename std::iterator_traits<Iterator>::value_type value_type;
            ...
            while (decimal < size)
            {
                ...
                } else if (compare(*mid, *(mid - 1)))
                {
                    ...
                    } else
                    {
                        ...
                        {
                            if (compare(*(buffer1.start - 1), *buffer1.start) ||
                                compare(*buffer1.start, *(buffer1.start - 1)))
                        ...
                        if (buffer_size <= cache_size)
                        {
                            ...
                            } else
                            {
                                ...
                                {
                                    if (compare(*buffer1.start, *(buffer1.start + 1)) ||
                                        compare(*(buffer1.start + 1), *buffer1.start))
                                        if (++count == buffer_size)
                                            break;
                                }
                                buffer1.end = buffer1.start + count;
                        ...
                        } else
                        {
                            ...
                            {
                                if (compare(*(buffer2.start - 1), *buffer2.start) ||
                                    compare(*buffer2.start, *(buffer2.start - 1)))
                                    if (++count == buffer_size)
                                        break;
                            }
                            ...
                            } else if (buffer1.length() == buffer_size)
                            {
                                ...
                                {
                                    if (compare(*buffer2.start, *(buffer2.start + 1)) ||
                                        compare(*(buffer2.start + 1), *buffer2.start))
                                        if (++count == buffer_size)
                                            break;
                                }
                                buffer2.end = buffer2.start + count;
                    ...
                    for (Iterator index = buffer1.start, indexA = firstA.end + 1;
                         indexA < blockA.end; index++, indexA += block_size)
                    {
                    ...
                    Range blockB = Range(B.start,
                                         B.start + std::min<size_t>(block_size, B.length() - bufferB.length()));
